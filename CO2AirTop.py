...
def dxCO2Air(CO2Air, CO2Top):
    ...
    #print('dx = ', (MCBlowAir + MCExtAir + MCPadAir - MCAirCan - MCAirTop - MCAirOut) / capCO2Air)
...
def euler(CO2Air_0, CO2Top_0, h, time):
    ...
    for idx in range(1, n + 1):
        ...
        if idx % 300 == 0:
            ...
            worksheet.write(row+1, 1, CO2Air)
            worksheet.write(row+1, 2, CO2Top)
            global TAir_0, TTop_0, TOut_0, vWind_0, URoof_0
            TAir_0 = float(df.at[row, 'TAir'])
            TTop_0 = float(df.at[row, 'TTop'])
            TOut_0 = float(df.at[row, 'TOut'])
            vWind_0 = float(df.at[row, 'vWind'])
            URoof_0 = float(df.at[row, 'URoof'])
...
def rk4(CO2Air_0, CO2Top_0, h, time):
    ...
    for idx in range(1, n + 1):
        ...
        if idx % 300 == 0:
            row = idx // 300
            worksheet.write(row+1, 4, CO2Air)
            worksheet.write(row+1, 5, CO2Top)
            global TAir_0, TTop_0, TOut_0, vWind_0, URoof_0
            TAir_0 = float(df.at[row, 'TAir'])
            TTop_0 = float(df.at[row, 'TTop'])
            TOut_0 = float(df.at[row, 'TOut'])
